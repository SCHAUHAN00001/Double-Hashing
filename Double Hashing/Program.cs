...
namespace Double_Hashing
{
    internal class Program
    {
        class HashLinear_Prob
        {
            ...
            public int hash2code(int element)
            {
                int prime = 7; 
                return prime - (element % prime);
            }

            public int doubleHash(int element)
            {
                int i = hashcode(element);
                int j = 0;
                int step = 1;
                int hash2 = hash2code(element); 

                while (hashtable[(i + j * hash2) % hashtablesize] != 0)
                {
                    j = (j + step) % hashtablesize;
                    step++;
                }

                return (i + j * hash2) % hashtablesize;
            }
